...
var hui = (function(selector, undefined){
	...
	huibase.iconToast = function(msg, icon){
		...
		if(iconToast.length < 1){
			...
			div.innerHTML = '<div id="hui-icons" class="hui-icons"></div><div id="hui-text-center" class="hui-text-center"></div>';
			document.body.appendChild(div);
	...
	huibase.prompt = function(msg, btnName, callBack, placeholder,callBack2){
		if(!btnName){btnName = ['取消','确定'];}
		if(!placeholder){placeholder = '';}
		hui.dialogBase();
		hui.dialogDom.innerHTML = '<div id="hui-dialog-in" style="width:300px;"><div id="hui-dialog-msg" style="padding-bottom:12px;">'+msg+'</div><div id="hui-dialog-input-in"><input type="text" id="hui-dialog-input" placeholder="'+placeholder+'" /></div><div id="hui-dialog-btn-line"><div>'+btnName[0]+'</div><div>'+btnName[1]+'</div></div></div>';
		var btns = document.getElementById('hui-dialog-btn-line').getElementsByTagName('div');
    ...
    huibase.promptLogin = function(msg, btnName, placeholder1, placeholder2, callBack, callBack2){
        if(!btnName){btnName = ['取消','确定'];}
        if(!placeholder1){placeholder1 = '';}
        if(!placeholder2){placeholder2 = '';}
        hui.dialogBase();
        hui.dialogDom.innerHTML = '<div id="hui-dialog-in" style="width:300px;"><div id="hui-dialog-msg" style="padding-bottom:12px;">'+msg+'</div><div id="hui-dialog-input-in-1"><input type="text" id="hui-dialog-input-1" placeholder="'+placeholder1+'" /></div>' +
			'<div id="hui-dialog-input-in"><input type="password" id="hui-dialog-input-2" placeholder="'+placeholder2+'" /></div><div id="hui-dialog-btn-line"><div>'+btnName[0]+'</div><div>'+btnName[1]+'</div></div></div>';
        var btns = document.getElementById('hui-dialog-btn-line').getElementsByTagName('div');
        btns[0].onclick =function(){
            hui.dialogClose();
            if(callBack2){
        ...
        btns[1].onclick = function(){
            if(callBack){callBack(document.getElementById("hui-dialog-input-1").value,document.getElementById("hui-dialog-input-2").value);}
            hui.dialogClose();
        };
    };
	/* actionSheet */
	huibase.actionSheet = function(meuns, cancel, callBack){
		hui.maskShow();
		...
		var actionSheets = '<ul>';
		for(var i = 0; i < meuns.length; i++){actionSheets += '<li huiASId="'+i+'">'+meuns[i]+'</li>';}
