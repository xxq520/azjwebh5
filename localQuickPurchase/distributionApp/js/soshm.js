...
/***/ function(module, exports, __webpack_require__) {
	...
	soshm.weixinSharetip = function(duration) {
	  ...
	  //alert("device.isWeixin: " + device.isWeixin);
	  if (device.isWeixin || device.isQQ) {
	    ...
	    //判断如果是微信小程序打开的话就则不显示请点击右上角的分享按钮提示
	    try {
	...
	function shareTo(site, data) {
	  ...
	  if (site === 'qzone' || site === 'qq') {
	    ...
	    // 在微信里点QQ分享，弹出右上角提示
		  if (device.isWeixin && (site.indexOf('qq') !== -1)) {
	  ...
	  if (site.indexOf('weixin') !== -1) {
		//layer.open({content:'请使用浏览器菜单中分享按钮，选择分享至微信', skin:'msg', time:5});
		//hui.alert('应用没权限调用微信，请选择其他分享方式(如QQ)');
