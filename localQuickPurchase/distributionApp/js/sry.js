...
if((shareSeq==""||shareSeq==null)&&enterpriseShareSeq!=null){
    setCookie("shareSeq",enterpriseShareSeq,180);
...
function addUser(){
    ...
    data.referrerMobile = referrerMobile;
...
if(generalAgent == null || generalAgent == ""){
    generalAgent = getCookie("generalAgent");
}
    ...
    $.ajax({
        ...
        async : true,
            ...
            if(code == 200){
                ...
                var nname  = data.data.virtualShop.nickName;
                setCookie("headImg", headImg, 7);
                setCookie("nickName", nname, 7);
...
function loginSkip(data){
    var userSeq = data.data.seq;
    ...
    if(enterpriseShareSeq != null && enterpriseShareSeq != "") {
        ...
        addUserBindingByShare(userSeq,enterpriseShareSeq);
        //var investUrl = "/localQuickPurchase/distributionVA/enterpriseLevelAgent?shareSeq="+enterpriseShareSeq;
        var investUrl = "/localQuickPurchase/distributionVA/agentInformation?distributorSeq="+userSeq+"&upgradeType=2";
        location.href=investUrl
        return;
    }
    var distributorType = getCookie("distributorType");
    ...
    var oAuth2AccessToken = data.data.oAuth2AccessToken;
    try{
    ...
    if(loginRetrunUrl == ""){
        ...
        //进入供货商邀请页面
        if(generalAgent != null && generalAgent != ""){
            window.location.href="/localQuickPurchase/distributionVA/store/generalAgent?seq="+generalAgent;
            return;
        }
        if(returnAddressUrl.indexOf("/passwordLogin")>=0){
            history.go(-2);
        }else if(returnAddressUrl=="" || returnAddressUrl.indexOf("login")>0
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data.code == "501"){
                ...
                $("#imgCode img").attr("src","/localQuickPurchase/validateCode?mobile="+mobile+"&random="+ random);
...
function refreshImgCode(mobile){
    $("#imgCode").on("click",function(){
        ...
        $(this).find('img').attr("src","/localQuickPurchase/validateCode?mobile="+mobile+"&time="+ new Date().toLocaleTimeString())
    })
}
    ...
    $.ajax({
        ...
        success : function(data) {
            ...
            if(code == 200){
                ...
                //}
                login(mobile,pwd);
                //code == 400 的时候，表示推荐人手机号有误或者该推荐人角色达不到绑定条件
            } else{
                // 因为注册接口有时候请求会失败，所以这里进行判断，如果返回的错误信息是请求失败，则在重新调用一次注册接口
                if(message.indexOf("请求失败") > 0){
                    addUserTwo();
...
function addUserBindingByShare (userSeq,enterpriseSeq){
    $.ajax({
        type : "POST",
