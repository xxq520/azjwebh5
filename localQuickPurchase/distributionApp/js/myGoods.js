...
$(function(){
	...
	var initData={
			//参数
			"param":function(pi){
				var param={"seq":seq,"pageSize":10,"pageIndex":pi,"keyWord":keyWord,"categoryId":categoryId};
				return param;
			},
			// 分类搜索参数
			"category":function(pi){
				var category={"seq":seq,"pageSize":10,"pageIndex":pi,"categoryId":categoryId};
				return category;
			},
			"functions":{
				"init" :function(data){
					...
					/*获取收藏夹列表*/
						...
						for(var n=0;n<goods.length;n++){
							var goodsdate = goods[n];
							var distributionPrice = goodsdate.goodsProStandard[0].distributionPrice;
							var stock = goodsdate.goodsProStandard[0].stock;
							_listhtml+="<div class='listitem' value='"+goodsdate.goodsId+"'>";
							_listhtml+="<a href='#'>";
							_listhtml+="<div class='mui-tab-link li-left' target='javascript:;'>";
							_listhtml+="<div class='lil-img'><img src='"+goodsdate.thumbnail+"' onerror='this.src=\"/localQuickPurchase/imgback/goods.jpg\"' /></div>";
							_listhtml+="<div class='lil-ifon'>";
							_listhtml+="<p class='lili-tit'>"+goodsdate.goodsName+"</p>";
							_listhtml+="<p class='lili-price' >分销价:"+(distributionPrice).toFixed(2)+"元/"+goodsdate.unit+"</p>";
							_listhtml+="<p class='lili-stock'>库存量："+(stock == null ? 0 : stock)+"</p>";
							//_listhtml+="<p class='lili-sales'>销售量：25468条</p>";
							_listhtml+="</div></div>";
							_listhtml+="</a>";
							_listhtml+="<div class='li-right'>";
							_listhtml+="<div class='mui-tab-method lir-share div-click' data-size='"+distributionPrice+"' data-id='"+goodsdate.goodsId+"' data-state='"+goodsdate.state+"' onclick='changeState(this);'>"+(goods[n].state==2?"下架":"上架")+"</div>";
