...
// 获取商品详情
function goodsInit(goodsInitId) {
    // 获取当前省份
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (goods != null) {
                    ...
                    var goodsProStandard = goods.goodsProStandard;
                    showCommission(goods.discounts, goods.discounts);
                    // ----------------- 2018.5.16 新版详情------------- begin --------------------
...
//				console.info(proStandardsMap);
                    ...
                    var endTime;
                    if (field != null) {
                        countDownText = field.countDownText;
                        intervalText = field.intervalText;
                        beginTime = field.seckillFieldBeginTime;
                        endTime = field.seckillFieldEndTime;
                    ...
                    //获取下单优惠价
                    var getDisProfit = goodsPsd.distributionProfit;
                    var getAgentProfit = goodsPsd.agentProfit;
                    showCommission(getDisProfit, getAgentProfit);
                    ...
                    for (var i = 0; i < attrList.length; i++) {
                        ...
                        for (var j = 0; j < attrValues.length; j++) {
                            ...
                            var sku = attrValue.sku;
                            chooseSpec += '<div class="spec-item" data-prime=' + prime + ' data-sku="' + sku + '"  onclick="selectSpces(this)" >' + attrValue + '</div>';
                        }
                        chooseSpec += '</div>';
                    }
                    $(".spec-box").html(chooseSpec);
                    ...
                    if (goodsImg == null || goodsImg.length <= 0) {
                        ...
                        for (var i = 0; i < goodsImg.length; i++) {
                            ...
                            if (i == 0 && null != goods.video
                                && null != goods.video.videoUrl && 1 == goods.video.auditStatus) {
                                var addvideo = "";
                                addvideo = '<div class="videoss"><div class="zhezhao"></div><div class="vediotav"><p>商品播放</p><img src="/localQuickPurchase/distributionApp/images/vedio/6.png" class="closes"  ></div><div class="zyvideo">  <video id="myVideo" class="video-js1"> <source src="' + goods.video.videoUrl + '" type="video/mp4"> </video> </div></div><img src="/localQuickPurchase/distributionApp/images/vedio/5.png" alt="" class="bofang" id="bofang">';
                                $(".imgBox").append(addvideo);

                                // zymedia('video',{autoplay: true});

                                var myPlayer = $("#myVideo")[0]
                                ...
                                $(".closes").click(function () {
                                    $(".videoss").css("display", "none")
                                    myPlayer.pause();
                                })
                                $(".zhezhao").click(function () {

                                    // console.log(myPlayer.paused)
                                    if (myPlayer.paused) { //如果已暂停则播放
                                        myPlayer.play(); //播放控制
                                    } else { // 已播放点击则暂停
                                        myPlayer.pause(); //暂停控制
                                    }
...
function getPriceForSelectSku(sku) {
    for(var i = 0; i < proStandardsMap.length; i++) {
        ...
        if (getSku == sku) {
            ...
            distributionStock = getProStandard.activityQuantity;
            $(".goodsImg").find("img").attr("src", specificationImg);
            $('.stock-num').html(getSalesVolume);//起卖量
            $(".slt-price").html("特卖价:￥" + (getDisPrice));//分销价
            $(".distributionPrice").html(getDisPrice);
            $(".tariffFee").html(tariffFee);//关税
            $("input[name=numValue]").val(getSalesVolume);//
            $(".distributionPrice").html(getDisPrice);
            if(getProStandard.goodsPrice != null && getProStandard.goodsPrice > getDisPrice){
                $(".goodsPrice").html("￥"+getProStandard.goodsPrice);
            }else{
                $(".goodsPrice").html("");
            }
            //分销商品库存赋值
            ...
            var getProfitPrice = getProStandard.agentProfit;
