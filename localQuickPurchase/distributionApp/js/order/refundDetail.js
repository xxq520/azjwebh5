...
var partialRefund =  getQueryString("partialRefund");//是否部分退款
var seq = seq;
...
if(partialRefund == "true"){
    $(".after-text").attr("placeholder","请填写详情售后信息");
}
if(route == 2){
...
$(document).on('click','.footer-btn',function(){
	...
	/*if (Courier == "") {
		...
		if (Courier == "有快递单号") {*/
			/*if (afterOrderno != "") {*/
	//var partialRefund =  $("#partialRefund").val();//是否部分退款
	var skuParam =  [sku];//是否部分退款
	if (afterWhy != "" || bankvalue == 8) {
		if (afterOrderno != "") {
			var bool = checkNum(afterOrderno);
			if (bool) {
				var datas = {"seq":seq,"orderno":orderno,"afterOrderno":afterOrderno,"afterType":saleType,"afterState":afterState,"afterWhy":afterWhy,
					"afterReason":afterReason,"supplierSeq":supplierSeq,"route":route,"imgIdArr":imgIdArr,"partialRefund":partialRefund,"listSku":skuParam};
