...
function goodsList(){
    $.ajax({
        ...
        data:JSON.stringify({"seq" : seq,"pageIndex" : pageIndex,"messageType":3}),
        dataType: "json",	//设置返回值得类型
...
function downMove(){
    $.ajax({
        ...
        url:"/localQuickPurchase/systematicNotification/findSysByMessageType",
        dataType : "json",//设置返回值得类型
        data:JSON.stringify({"seq" : seq,"pageIndex" : 1,"messageType":3}),
        dataType: "json",	//设置返回值得类型
        success:function(data){
            var html = '';
            if(data.code == 200){
                var messageTopic = data.data.list;
                if(messageTopic == null || messageTopic.length == 0){
                    html = '<p class="positionContent">暂时没有培训信息!</p>';
