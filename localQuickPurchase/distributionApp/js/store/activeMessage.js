...
//获取ModelAndView的值
var seq = $("#method1").val();
var userType = $("#method2").val(); //用户类型
var messageType = $("#method3").val(); //信息类型
var informId = $("#method4").val(); //系统id
...
function goodsList(){
    $.ajax({
        ...
        data:JSON.stringify({"seq" : seq,"pageIndex" : pageIndex,"messageType":4}),
        dataType: "json",	//设置返回值得类型
...
function downMove(){
    $.ajax({
        ...
        url:"/localQuickPurchase/systematicNotification/findSysByMessageType",
        dataType : "json",//设置返回值得类型
        data:JSON.stringify({"seq" : seq,"pageIndex" : 1,"messageType":4}),
        dataType: "json",	//设置返回值得类型
        success:function(data){
            var html = '';
            if(data.code == 200){
                var messageTopic = data.data.list;
                if(messageTopic == null || messageTopic.length == 0){
                    html = '<p class="positionContent">暂时没有活动信息!</p>';
...
//修改状态
$(document).on('click','.notice_text',function(){
