...
function huiSwipe(selector){
	...
	this.itemImg     = this.swipe.find('.hui-swipe-item img');
	...
	_selfObj.swipe.css({height : this.width+'px'});
	this.swipe.find('img').css({height : this.width+'px'});
	...
	this.changeIndicator = function(index){
		...
		_selfObj.indicator.find('.hui-fr').html(index+"/"+this.itemSize);
	};
	this.swpieMove = 0;
