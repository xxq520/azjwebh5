...
function refreshInvestmentGoods() {
    ...
    html2 += '<img src="/goods/images/set-add.png"/>';
    html2 += '</div>';
    var t = isLoad();
    console.info(t);
    if (!t) {
    ...
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '/goods/investmentGoods/findAllInvestmentGoods',
        data: {
            "seq": seq,
            "pageIndex": 1,
            "pageSize": 10
        ...
        success: function (data) {
            ...
            if (data.code == 1000) {
                ...
                if (storeHeadImg == null) {
                    storeHeadImg = "/goods/images/head.png";
                }
                ...
                if (distributionGoodsList != null && distributionGoodsList.length > 0) {
                    for (var i = 0; i < distributionGoodsList.length; i++) {
                        ...
                        if (goodsImgPath == null || goodsImgPath == "") {
                            goodsImgPath = "/goods/images/default-classify.png";
                        } else {
                        ...
                        html += '<img id=' + goodsId + "," + goodsName + ' class="set_del" src="/goods/images/set-close.png" onclick="clickGoods(this)" />';
...
function clickGoods(obj) {
    ...
    hui.confirm("" + goodsName + ",您确定删除此商品么", ['取消', '确定'], function () {
        $.ajax({
            type: 'POST',
            url: '/goods/investmentGoods/deleteInvestmentGoods',
            data: {
                seq: seq,
                goodsId: goodsId
            },
...
function getGenreId() {
    ...
    $.ajax({
        type: 'GET',
        dataType: 'json',
        url: '/goods/goodsCategory/getGenreId?seq=' + seq,
        data: {},
        async: false,
        success: function (data) {
            var list = data.data;
...
var local_url = '/goods/investmentGoods/findLocaleRecruitGoodsVOBySeq';
//分类商品下拉刷新加载
...
function getDataHtml(data) {
    ...
    html += '<img class="img-left" id="' + goodsId + '" src="' + goodsImgPath + '" onerror=this.src="/goods/images/default-classify.png" />';
    html += '<div class="img-right" id="' + goodsId + '" onclick="genreIdGoodsAdd(this)">';
    html += '<img src="/goods/images/set-add2.png" />';
    html += '</div>';
...
function genreIdGoodsAdd(obj) {
    ...
    add_html += '<img id="' + goodsId + '" src="/goods/images/set-close.png" onclick="del(this)" />';
    add_html += '</li>';
...
function saveInvestmentGoods() {
    ...
    $.ajax({
        type: 'POST',
        url: '/goods/investmentGoods/saveInvestmentGoods',
        dataType: 'json',
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.code == 1000) {
                ...
                setTimeout(function () {
                    //跳转到现场招商商品页面
                    window.location.href = "setGoods.html";
                }, 500)
...
$(document).on('click', '.goodsDetails', function () {
    ...
    var url = "/goods/goodsDetail/" + goodsId + "/0" + "/" + seq;
